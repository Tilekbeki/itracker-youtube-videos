---
created at: 2025-08-19
tags: " "
zero-links:
  - 00 Идеи для видео
scheduled-at:
completed-at:
статус:
  - идея
---
# Обложка
ссылка и текст
# Сценарий

## хук
Текст
## тело
число %10 вернет последнее цифру

value ?? '' если value null или undefined nullish coalescing помогает нам вернуть другое значение


деструкторизация меняем местами
[a,b] = [b,a]
## призыв к действию

Если ты тоже мечтаешь войти в IT – подпишись. Я буду делиться каждым этапом и делиться пользой.  
Оставайся, если хочешь узнать – получится ли у меня дойти до конца.

## описание видео


## теги к видео

## рилс версия 

текст
### музыка
название или ссылка# Сценарий

## хук
Текст
## тело
Текст
## призыв к действию

Если ты тоже мечтаешь войти в IT – подпишись. Я буду делиться каждым этапом и делиться пользой.  
Оставайся, если хочешь узнать – получится ли у меня дойти до конца.

## описание видео


## теги к видео

## рилс версия 

текст
### музыка
название или ссылка